...
# The PDO is targetted for an SQL upload.
#
...
#
# USAGE: extract-testdata-for-sql job-name zip-file -pid=nnnn -eid=nnnn -bid=aaaa-nnnn
# Where: 
...
#   It is your responsibility to know what the next new pid, eid and bid should be!
#
...
print_namespace_update_usage() {
	echo " USAGE: extract-testdata-for-sql.sh job-name zip-file -pid=nnnn -eid=nnnn -bid=aaaa-nnnn"
	echo " Where:" 
    ...
    echo "   -pid=nnnn gives the starting number for patient identifiers"
    echo "   -eid=nnnn gives the starting number for event identifiers"
